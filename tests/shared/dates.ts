 ...
 * @returns void
 */
export let mineBlock: (timestampSec: number) => Promise<void> = (
  timestampSec,
) =>
  Promise.reject(
    new Error("mineBlock not initialized. Call initWeb3Time first"),
  );
 ...
 * @returns number
 */
export let blockTimestamp: (block?: number) => Promise<number> = (block) =>
  Promise.reject(
    new Error("blockTimestamp not initialized. Call initWeb3Time first"),
  );
export function initWeb3Time(web3: Web3) {
  ...
  blockTimestamp = async (blockNumber) => {
    ...
    return typeof block.timestamp == "string"
      ? Number.parseInt(block.timestamp)
      : block.timestamp;
  };
}
