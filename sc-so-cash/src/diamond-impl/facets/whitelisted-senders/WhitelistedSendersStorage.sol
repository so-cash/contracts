...
library WhitelistedSendersStorage {
  // Layout struct holds the state variables for the Diamond facet.
  struct Layout {
    mapping(address => bool) _whitelistedSenders;
  }

  // Unique storage slot for PathFinderStorage.
  bytes32 internal constant STORAGE_SLOT = keccak256("so-cash.whitelisted-senders.storage");
