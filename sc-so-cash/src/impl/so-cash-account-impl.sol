...
contract SoCashAccount is ISoCashAccount, ISoCashOwnedAccount, WhitelistedSenders, HTLCPaymentCapacity {
  ...
  mapping(bytes32 => address) private _attributesAddr;
  ...
  function getAttributeAddr(bytes32 _name) public view returns (address) {
      return _attributesAddr[_name];
  }
  // TODO: Limit to only some used to set the attributes, client should not be allowed to do it
  function setAttributeAddr(bytes32 _name, address value) public onlyWhitelisted {
      _attributesAddr[_name] = value;
  }
  ...
  function transferEx(RecipentInfo calldata recipient, uint256 amount, string calldata details) public override returns (bool) {
    if (isWhitelisted(_msgSender())) {
      ...
      // TODO: Attention, there is a security issue here, the caller can be an allowed wmart contract but called by a non allowed person
      // For instance in the case of a FXProvider calling the transferEx function, the FXProvider is allowed but the caller is not as the account is given in parameter
  ...
  function transferLockedFunds(bytes32 key, RecipentInfo calldata recipient, string calldata secret, string calldata details) public returns (bool) {
    ...
    // TODO: remove the recipient from the parameters and use one in the htlc structure so transfer can only be done to the pre-defined recipient
