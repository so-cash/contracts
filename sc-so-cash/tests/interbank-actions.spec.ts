...
describe("Test SoCash Inter Bank Functions", async function () {
  ...
  it("Can transfer using an IBAN", async () => {
    ...
    let execPlan = await g.bank1.simulateTransfer(
      g.bo1User.call(),
      account1.deployedAt,
      receipientInfo(undefined, undefined, iban),
      300_000,
    );
    console.log("Exec plan", cleanStructAndMap(execPlan));

    // Simulate the interbank first
    execPlan = await g.bank1.simulateInterbankTransfer(
      g.bo1User.call(),
      account1.deployedAt,
      receipientInfo(undefined, undefined, iban),
      300_000,
    );
    console.log("Exec interbank plan", cleanStructAndMap(execPlan));
