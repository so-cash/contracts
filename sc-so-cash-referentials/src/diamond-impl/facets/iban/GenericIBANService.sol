...
import {SharedFunctions} from "@so-cash/sc-so-cash/src/diamond-impl/libs/SharedFunctions.sol";
import {
    CodeType,
    BankIdentifier
} from "../../../intf/so-cash-referential.sol";
import {CountryStateManagementInternal} from "../../CountryStateManagement.sol";
...
contract Generic_IBANServiceInternal {
...
contract Generic_IBANServiceFacet is I_IBANService, Generic_IBANServiceInternal, CountryStateManagementInternal {
  ...
  function _decodeIBAN(string memory iban) internal pure returns (bool, CountryCode , string[] memory ) {
  ...
  function decodeIBAN(string memory iban) external pure returns (bool, CountryCode , string[] memory ) {
    return _decodeIBAN(iban);
  }

  function decodeIBANToContracts(string memory iban) external view returns (bool , address bank, address account) {
    (bool valid, CountryCode country, string[] memory codes) = _decodeIBAN(iban);
    if(codes.length != 3 || !valid) {
      return (false, address(0), address(0));
    }
    if(CountryCode.unwrap(country) != _countryCode()) {
      return (true, address(0), address(0));
    }
    bytes3 currency = SharedFunctions.stringToBytes3(codes[2]);
    CodeType[] memory bankCodes = new CodeType[](2);
    bankCodes[0] = CodeType.wrap(SharedFunctions.stringToBytes10(codes[0]));
    bankCodes[1] = CodeType.wrap(SharedFunctions.stringToBytes10(codes[1]));
    // BankIdentifier memory bankId = BankIdentifier(CountryCode.unwrap(country), bankCodes);
    ISoCashBankExternal bankModule = _getBankModule(bankCodes, currency);
    if(address(bankModule) == address(0)) {
      return (true, address(0), address(0));
    }
    account = address(bankModule.addressOfFullAccount(codes[2]));
    return (true, address(bankModule), account);
  }
